...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None, return_attention=False):
        B, T, C = x.size()
        ...
        attention_map_to_return = att if return_attention else None
        ...
        if return_attention:
            return y, attention_map_to_return
        else:
            return y, None 
...
class Block(nn.Module):
    ...
    def forward(self, x, layer_past=None, return_present=False, return_attention=False):
        if return_attention:
            attn_output, attention_map = self.attn(self.ln1(x), return_attention=True)
            x = x + attn_output
...
class GPT(nn.Module):
    ...
    def forward(self, idx, embeddings=None, targets=None):
        if embeddings is not None:
            ...
            raise ValueError("Either 'idx' or 'embeddings' must be provided to the forward pass.")
        
        # t_shape [B, seq_len ,num_embd] --> seq_len
        t = token_embeddings.shape[1]
        assert t <= self.block_size, f"Cannot forward, model block size is exhausted. Input sequence length ({t}) exceeds block size ({self.block_size})."
        
        position_embeddings = self.pos_emb[:, :t, :] # each position maps to a (learnable) vector
        # [B, seq_len, num_embed] --> [B, seq_len, num_embed]
        x = self.drop(token_embeddings + position_embeddings)
        # [B, seq_len, num_embed] --> [B, seq_len, num_embed]
        x = self.blocks(x)
        x = self.ln_f(x)
        # [B, seq_len, num_embed] --> [B, seq_len, vocab_size]
        logits = self.head(x)
    ...
    def forward_for_visualization(self, idx, embeddings=None):
        """
        A method specifically designed for forward propagation and extracting attention maps from all layers.
        """
        if embeddings is not None:
            token_embeddings = embeddings
        elif idx is not None:
